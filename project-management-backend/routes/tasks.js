...
// In-memory taskColumns for tasks (replace with DB later)
const taskColumns = {
    'todo': {
        title: 'To Do',
        color: '#6b7280',
...
let nextTaskId = 10; // Start from 10 since we have 9 tasks already

const getNextId = () => nextTaskId++;

// Helper function to find task by ID across all columns
...
router.get('/:id', (req, res) => {
    const result = findTaskById(parseInt(req.params.id));
    if (!result) {
...
router.delete('/:id', (req, res) => {
    ...
    const taskIndex = taskColumns[columnKey].tasks.findIndex(t => t.id === parseInt(req.params.id));
    taskColumns[columnKey].tasks.splice(taskIndex, 1);
